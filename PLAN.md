...
2. Реализовать POST-эндпоинт, который:
   - Принимает `content` (текст статьи) и опционально `title` и `date` из тела запроса
   - Использует OpenRouter API
   - Отправляет промпт: "Создай пост для Telegram на русском языке на основе этой статьи. Пост должен быть кратким, информативным и привлекательным. Используй эмодзи для оформления. Включи основные идеи и призыв к действию."
   - Возвращает готовый пост для Telegram
3. Добавить обработку ошибок
4. Использовать fallback на другие модели
...
2. Для каждого действия:
   ...
   - Передать `content` (и для telegram - также `title` и `date` если есть)
...
3. **API для "Пост для Telegram"** - требует дополнительных данных (title, date)
