...
- Корректно передает данные в API (для telegram - title и date)
...
- API принимает опциональные параметры `title` и `date`
- Фронтенд правильно передает title и date в API
...
- **Telegram:** Привлекательность, использование эмодзи, призыв к действию
